...
export function buildCheckoutAttemptId(prefix = "chk") {
  return `${prefix}_${Date.now()}_${randomUUID().replace(/-/g, "").slice(0, 18)}`;
}
...
export function buildPlanSnapshot(plan: any) {
  return {
    planId: plan._id,
    code: plan.code,
    name: plan.name,
...
export function sanitizePayment(payment: any) {
  const userId =
    payment.userId?._id != null ? String(payment.userId._id) : String(payment.userId);
